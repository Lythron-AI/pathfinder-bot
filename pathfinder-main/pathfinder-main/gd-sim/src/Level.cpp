...
Player& Level::runFrame(bool pressed, float dt) {
	...
	int totalSections = static_cast<int>(sections.size());
	int sectionIdx = std::clamp(static_cast<int>(p.pos.x / sectionSize), 0, totalSections - 1);
	
	std::vector<std::vector<ObjectContainer>*> activeSections;
	activeSections.reserve(3);

	if (sectionIdx > 0) activeSections.push_back(&sections[sectionIdx - 1]);
	activeSections.push_back(&sections[sectionIdx]);
	if (sectionIdx + 1 < totalSections) activeSections.push_back(&sections[sectionIdx + 1]);
